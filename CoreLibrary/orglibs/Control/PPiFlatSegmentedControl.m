...
- (void)updateItems
{
    ...
    //Generating segments
    float buttonWith=ceil(self.frame.size.width / self.items.count);
    int i=0;
    for(PPiFlatSegmentItem *item in self.items){
        NSString *text=item.title;
        NSObject *icon=item.icon;
        
        UIAwesomeButton  *button;
        if([icon isKindOfClass:[UIImage class]]) {
            button = [[UIAwesomeButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, self.frame.size.height) text:text iconImage:(UIImage *)icon attributes:@{} andIconPosition:self.iconPosition];
        }
        else {
            button = [[UIAwesomeButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, self.frame.size.height) text:text icon:(NSString *)icon attributes:@{} andIconPosition:self.iconPosition];
        }
...
//        self.layer.borderColor=self.borderColor.CGColor;
        self.backView.layer.borderWidth=self.borderWidth;
        self.backView.layer.borderColor=self.borderColor.CGColor;
//        self.backView.backgroundColor = self.borderColor;
    }
    else {
//        self.layer.borderWidth=0;
    ...
    if (self.cornerRadius) {
        self.layer.cornerRadius = self.backView.layer.cornerRadius = self.cornerRadius;
    }else{
        self.layer.cornerRadius = self.backView.layer.cornerRadius = segment_corner;
    }
...
//            segment.layer.borderWidth = self.borderWidth;
//            segment.layer.borderColor = self.selectedColor.CGColor;
        }
        else{
            //Non selected
